...
class User(db.Model):
    ...
    account_created_on = db.Column(
        db.DateTime, 
        default=db.func.now()
    )

    account_cancelled_on = db.Column(
        db.DateTime, 
        default=None
    )

    agreed_to_tos = db.Column(
        db.Boolean, 
        default=True
    )

    account_created_from_ip = db.Column(
        db.String(45)
    )
